---
title: "Codes for prediction of Size"
output:
  html_document:
    df_print: paged
---
...
for (i in 1:3) {
  thres_gpd[i] <-
    quantile(epid_data_cum[epid_data_cum$weeks == i,]$t_inc_cent,
             probs = level_gpd[i],
             na.rm = T)
}
```
...
for (i in 1:(d)) {
  week <-
    epid_data_cum[(epid_data_cum$weeks == i) &
                    !is.na(epid_data_cum$t_inc_cent), ]$t_inc_cent
  univ.fit_exp[[i]] <-
    fevd(
      x = week,
...
qq.plot <-
  ggplot(data = df_qq, aes(x = Obs, y = Theo)) + geom_point(shape = 1, size = 3)
qq.plot <-
  qq.plot + geom_abline(
    slope = 1,
    intercept = 0,
...
qq.plot  <-
  qq.plot + xlim(0, 6685) + ylim(0 , 6685) + coord_fixed()
...
### Table 5b) and Saving the parameters
```{r table 5b}
...
x12 <-
  2 * c(epid_data_cum2019[epid_data_cum2019$weeks == 1, ]$excess_stand,
        epid_data_cum2019[epid_data_cum2019$weeks == 2, ]$excess_stand)
...
for (i in 1:length(thres_pred)) {
  proba.predict_GPD[, i] <-
    excessprobGumbelU_3g12(
      ...
      alpha = est.alpha_M1,
      beta = est.beta_M1
    ) * proba_extreme_3g12_cum(x12 = x12)
  reg_2019 <-
    data.frame(
...
## Brier plots (Figure 5 c) and d))

### Threshold = 0.5 $\times$ max = 816 (Figure 5c))
...
### Threshold = 0.75 $\times$ max = 1,224 (Figure 5d))

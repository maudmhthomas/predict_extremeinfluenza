...
res_week3 <- data.frame(res_parallel)
colnames(res_week3) <-
  c(
    "Week3",
    "Predict_Gumbel_05",
...
### Threshold = 0.95 $\times$ max = 1,551 (not shown in the paper)
...
### Threshold = 1.2 $\times$ max = 1,959 (not shown in the paper)
```{r true brierplot12}
res_week3_12 <-
...
```{r roc05}
dis.thres_LOGIT <- sort(res_week3_05$Predict_Logit_05)
roc.data_LOGIT <-
  data.frame(DisThres = dis.thres_LOGIT, Model = rep("Logistic", length(dis.thres_LOGIT)))
for (i in 1:length(dis.thres_LOGIT)) {
  roc.data_LOGIT$TP[i] <-
    sum((res_week3_05$Predict_Logit_05 > dis.thres_LOGIT[i]) &
          (res_week3_05$Outcome == 1),
        na.rm = T
    ) / sum(res_week3_05$Outcome == 1)
  roc.data_LOGIT$FP[i] <-
    sum((res_week3_05$Predict_Logit_05 > dis.thres_LOGIT[i]) &
          (res_week3_05$Outcome == 0),
        na.rm = T
    ) / sum(res_week3_05$Outcome == 0)
}


dis.thres_MGPD <- sort(res_week3_05$Predict_Gumbel_05)
roc.data_MGPD <-
...
for (i in 1:length(dis.thres_MGPD)) {
  roc.data_MGPD$TP[i] <-
    sum((res_week3_05$Predict_Gumbel_05 > dis.thres_MGPD[i]) &
          (res_week3_05$Outcome == 1)) / sum(res_week3_05$Outcome == 1)
  roc.data_MGPD$FP[i] <-
    sum((res_week3_05$Predict_Gumbel_05 > dis.thres_MGPD[i]) &
          (res_week3_05$Outcome == 0)) / sum(res_week3_05$Outcome == 0)
}

dis.thres_TRUE <- sort(res_week3_05$Predict_True_05)
roc.data_TRUE <-
  data.frame(DisThres = dis.thres_TRUE, Model = rep("True", length(dis.thres_TRUE)))
for (i in 1:length(dis.thres_TRUE)) {
  roc.data_TRUE$TP[i] <-
    sum((res_week3_05$Predict_True_05 > dis.thres_TRUE[i]) &
          (res_week3_05$Outcome == 1),
        na.rm = T
    ) / sum(res_week3_05$Outcome == 1)
  roc.data_TRUE$FP[i] <-
...
## AUC ###
auc_GP_05 <- auc(roc(res_week3_05$Outcome, res_week3_05$Predict_Gumbel_05))
auc_Logit_05 <- auc(roc(res_week3_05$Outcome, res_week3_05$Predict_Logit_05))
auc_True_05 <- auc(roc(res_week3_05$Outcome, res_week3_05$Predict_True_05))
...
```{r roc075}
dis.thres_LOGIT <- sort(res_week3_075$Predict_Logit_075)
roc.data_LOGIT <-
  data.frame(DisThres = dis.thres_LOGIT, Model = rep("Logistic", length(dis.thres_LOGIT)))
for (i in 1:length(dis.thres_LOGIT)) {
  roc.data_LOGIT$TP[i] <-
    sum((res_week3_075$Predict_Logit_075 > dis.thres_LOGIT[i]) &
          (res_week3_075$Outcome == 1),
        na.rm = T
    ) / sum(res_week3_075$Outcome == 1)
  roc.data_LOGIT$FP[i] <-
    sum((res_week3_075$Predict_Logit_075 > dis.thres_LOGIT[i]) &
          (res_week3_075$Outcome == 0),
        na.rm = T
    ) / sum(res_week3_075$Outcome == 0)
}


dis.thres_MGPD <- sort(res_week3_075$Predict_Gumbel_075)
roc.data_MGPD <-
  data.frame(DisThres = c(dis.thres_MGPD),
             Model = rep("Gumbel", length(dis.thres_MGPD)))
...
for (i in 1:length(dis.thres_MGPD)) {
  roc.data_MGPD$TP[i] <-
    sum((res_week3_075$Predict_Gumbel_075 > dis.thres_MGPD[i]) &
          (res_week3_075$Outcome == 1)) / sum(res_week3_075$Outcome == 1)
  roc.data_MGPD$FP[i] <-
    sum((res_week3_075$Predict_Gumbel_075 > dis.thres_MGPD[i]) &
          (res_week3_075$Outcome == 0)) / sum(res_week3_075$Outcome == 0)
}

dis.thres_TRUE <- sort(res_week3_075$Predict_True_075)
roc.data_TRUE <-
  data.frame(DisThres = dis.thres_TRUE, Model = rep("True", length(dis.thres_TRUE)))
for (i in 1:length(dis.thres_TRUE)) {
  roc.data_TRUE$TP[i] <-
    sum((res_week3_075$Predict_True_075 > dis.thres_TRUE[i]) &
          (res_week3_075$Outcome == 1),
        na.rm = T
    ) / sum(res_week3_075$Outcome == 1)
  roc.data_TRUE$FP[i] <-
...
## AUC ###
auc_GP_075 <- auc(roc(res_week3_075$Outcome, res_week3_075$Predict_Gumbel_075))
auc_Logit_075 <- auc(roc(res_week3_075$Outcome, res_week3_075$Predict_Logit_075))
auc_True_075 <- auc(roc(res_week3_075$Outcome, res_week3_075$Predict_True_075))

kable(data.frame(c(auc_GP_075, auc_Logit_075, auc_True_075)))
```

### Threshold = 0.95 $\times$ max = 1,551

```{r roc095}
dis.thres_LOGIT <- sort(res_week3_095$Predict_Logit_095)
roc.data_LOGIT <-
  data.frame(DisThres = dis.thres_LOGIT, Model = rep("Logistic", length(dis.thres_LOGIT)))
for (i in 1:length(dis.thres_LOGIT)) {
  roc.data_LOGIT$TP[i] <-
    sum((res_week3_095$Predict_Logit_095 > dis.thres_LOGIT[i]) &
          (res_week3_095$Outcome == 1),
        na.rm = T
    ) / sum(res_week3_095$Outcome == 1)
  roc.data_LOGIT$FP[i] <-
    sum((res_week3_095$Predict_Logit_095 > dis.thres_LOGIT[i]) &
          (res_week3_095$Outcome == 0),
        na.rm = T
    ) / sum(res_week3_095$Outcome == 0)
}


dis.thres_MGPD <- sort(res_week3_095$Predict_Gumbel_095)
roc.data_MGPD <-
  data.frame(DisThres = c(dis.thres_MGPD),
             Model = rep("Gumbel", length(dis.thres_MGPD)))
...
for (i in 1:length(dis.thres_MGPD)) {
  roc.data_MGPD$TP[i] <-
    sum((res_week3_095$Predict_Gumbel_095 > dis.thres_MGPD[i]) &
          (res_week3_095$Outcome == 1)) / sum(res_week3_095$Outcome == 1)
  roc.data_MGPD$FP[i] <-
    sum((res_week3_095$Predict_Gumbel_095 > dis.thres_MGPD[i]) &
          (res_week3_095$Outcome == 0)) / sum(res_week3_095$Outcome == 0)
}

dis.thres_TRUE <- sort(res_week3_095$Predict_True_095)
roc.data_TRUE <-
  data.frame(DisThres = dis.thres_TRUE, Model = rep("True", length(dis.thres_TRUE)))
for (i in 1:length(dis.thres_TRUE)) {
  roc.data_TRUE$TP[i] <-
    sum((res_week3_095$Predict_True_095 > dis.thres_TRUE[i]) &
          (res_week3_095$Outcome == 1),
        na.rm = T
    ) / sum(res_week3_095$Outcome == 1)
  roc.data_TRUE$FP[i] <-
...
## AUC ###
auc_GP_095 <- auc(roc(res_week3_095$Outcome, res_week3_095$Predict_Gumbel_095))
auc_Logit_095 <- auc(roc(res_week3_095$Outcome, res_week3_095$Predict_Logit_095))
auc_True_095 <- auc(roc(res_week3_095$Outcome, res_week3_095$Predict_True_095))
...
```{r true roc1}
dis.thres_MGPD <- sort(res_week3_1$Predict_Gumbel_1)
roc.data_MGPD <-
  data.frame(DisThres = c(dis.thres_MGPD),
             Model = rep("Gumbel", length(dis.thres_MGPD)))
...
for (i in 1:length(dis.thres_MGPD)) {
  roc.data_MGPD$TP[i] <-
    sum((res_week3_1$Predict_Gumbel_1 > dis.thres_MGPD[i]) &
          (res_week3_1$Outcome == 1)) / sum(res_week3_1$Outcome == 1)
  roc.data_MGPD$FP[i] <-
    sum((res_week3_1$Predict_Gumbel_1 > dis.thres_MGPD[i]) &
          (res_week3_1$Outcome == 0)) / sum(res_week3_1$Outcome == 0)
}

dis.thres_TRUE <- sort(res_week3_1$Predict_True_1)
roc.data_TRUE <-
...
for (i in 1:length(dis.thres_TRUE)) {
  roc.data_TRUE$TP[i] <-
    sum((res_week3_1$Predict_True_1 > dis.thres_TRUE[i]) &
          (res_week3_1$Outcome == 1)) / sum(res_week3_1$Outcome == 1)
  roc.data_TRUE$FP[i] <-
    sum((res_week3_1$Predict_True_1 > dis.thres_TRUE[i]) &
          (res_week3_1$Outcome == 0)) / sum(res_week3_1$Outcome == 0)
...
## AUC ###
auc_GP_1 <- auc(roc(res_week3_1$Outcome, res_week3_1$Predict_Gumbel_1))
auc_True_1 <- auc(roc(res_week3_1$Outcome, res_week3_1$Predict_True_1))

kable(data.frame(c(auc_GP_1, auc_True_1)))
```


### Threshold = 1.2 $\times$ max = 1,959

```{r true roc12}
dis.thres_MGPD <- sort(res_week3_12$Predict_Gumbel_12)
roc.data_MGPD <-
  data.frame(DisThres = c(dis.thres_MGPD),
             Model = rep("Gumbel", length(dis.thres_MGPD)))
...
for (i in 1:length(dis.thres_MGPD)) {
  roc.data_MGPD$TP[i] <-
    sum((res_week3_12$Predict_Gumbel_12 > dis.thres_MGPD[i]) &
          (res_week3_12$Outcome == 1)) / sum(res_week3_12$Outcome == 1)
  roc.data_MGPD$FP[i] <-
    sum((res_week3_12$Predict_Gumbel_1 > dis.thres_MGPD[i]) &
          (res_week3_12$Outcome == 0)) / sum(res_week3_12$Outcome == 0)
}


dis.thres_TRUE <- sort(res_week3_12$Predict_True_12)
roc.data_TRUE <-
  data.frame(DisThres = c(dis.thres_TRUE),
             Model = rep("True", length(dis.thres_TRUE)))
...
for (i in 1:length(dis.thres_TRUE)) {
  roc.data_TRUE$TP[i] <-
    sum((res_week3_12$Predict_True_12 > dis.thres_TRUE[i]) &
          (res_week3_12$Outcome == 1)) / sum(res_week3_12$Outcome == 1)
  roc.data_TRUE$FP[i] <-
    sum((res_week3_12$Predict_True_12 > dis.thres_TRUE[i]) &
          (res_week3_12$Outcome == 0)) / sum(res_week3_12$Outcome == 0)
...
## AUC ###
auc_GP_12 <- auc(roc(res_week3_12$Outcome, res_week3_12$Predict_Gumbel_12))
auc_True_12 <- auc(roc(res_week3_12$Outcome, res_week3_12$Predict_True_12))
